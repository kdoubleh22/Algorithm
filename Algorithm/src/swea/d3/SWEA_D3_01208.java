...
public class SWEA_D3_01208 {

	static BufferedReader input = new BufferedReader(new InputStreamReader(System.in));
	static StringBuilder output = new StringBuilder();
	static StringTokenizer tokens;
	
	static int findMax(int[] heights) {
	...
	static int findMin(int[] heights) {
	...
	static void dump(int[] heights, int maxIdx, int minIdx) {
		heights[maxIdx] -= 1;
		heights[minIdx] += 1;
	}

	public static void main(String[] args) throws IOException {
		input = new BufferedReader(new StringReader(src));//d
		for (int tc = 1; tc <= 10; tc++) {
			tokens = new StringTokenizer(input.readLine());
			int dumpNum = Integer.parseInt(tokens.nextToken());
			...
			for (int k = 0; k < dumpNum; k++) {
				...
				height = findMax(heights)-findMin(heights);
				if(height<=1) {
					break;
				}
				dump(heights,maxIdx,minIdx);
				height = findMax(heights)-findMin(heights);
//				if(height==0) {
//					break;
//				}
			} // k
			output.append(String.format("#%d %d%n", tc,height));
		} // tc
